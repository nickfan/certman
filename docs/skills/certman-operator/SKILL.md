...
- Scheduler 常驻或一次性调度
- 查询 job 状态，等待终态
- 按 target_scope 进行分段调度或查询（多环境/多集群）
- 管理 webhook（create/list/get/update/delete）
- 检查控制面健康
- 节点通道优化（events SSE / subscribe / heartbeat / callback）
- 准备 AI 接入（MCP / OpenAPI / skill）
- 检查或调整可选交付扩展（如 `aws-acm`、`k8s-ingress`）
...
9. 低延迟分发
- agent 端配置 `control_plane.prefer_sse=true`
- 优先走 `/api/v1/node-agent/events`（SSE），失败回退 `/subscribe`，再回退 `/poll`
- 如需关闭 SSE，设置 `control_plane.prefer_sse=false`

10. bundle token 故障恢复
- 默认 `[server].bundle_token_required=true`
- 节点必须使用 poll/subscribe 返回的 `bundle_token` 下载 bundle
- 若 token 过期，重新发起 poll/subscribe 获取新 token

11. 可选交付扩展核查
- `aws-acm`、`k8s-ingress` 属于可选扩展，不是默认主功能
- 只有当 `entries[].delivery_targets[]` 声明且 `enabled=true` 时才会执行
- 不要假设所有条目都一定存在后置分发
...
8. `certmanctl job list` 目前不支持 `--target-scope`；需要作用域过滤时用 REST 或 `certman-mcp` 的 `job_list(target_scope=...)`。
