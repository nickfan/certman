...
3. `events` is a signed SSE channel with `connected/assignment/timeout` events; prefer `control_plane.prefer_sse=true` when low-latency dispatch is needed.
4. recommended fallback chain is `events -> subscribe -> poll`. 
